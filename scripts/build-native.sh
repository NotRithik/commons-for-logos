#!/bin/sh
# Uses source checkouts already fetched and inspected. No network or installer.
set -eu
...
ctest --test-dir "$OUT/plugin" --output-on-failure
cmake --install "$OUT/plugin" --prefix "$PREFIX"
printf 'Installed tested native module at %s\n' "$PREFIX"
